...
<body style="font-family:Arial,sans-serif;color:#1a1530;font-size:17px;line-height:1.62;max-width:640px;margin:0 auto;padding:2rem 1rem;">
  ...
  const tokenValues: Record<string, string> = {
    ...
    '{{volunteer_role}}':        firstDefinedString(directContext.volunteer_role),
    '{{volunteer_shift_dates}}': firstDefinedString(directContext.volunteer_shift_dates),
    '{{volunteer_approved_hours}}': firstDefinedString(directContext.volunteer_approved_hours),
    '{{volunteer_point_person}}': firstDefinedString(directContext.volunteer_point_person),
    '{{volunteer_email}}':       firstDefinedString(directContext.volunteer_email, performerEmail),
  ...
  const rawHtmlSnippets = Object.entries(tokenValues)
    .filter(([token, value]) => (token.endsWith('_button}}') || rawHtmlTokens.has(token)) && value)
    .map(([, value]) => value);
