...
9. Generated URL slugs can use only lowercase letters, numbers, and hyphens.
10. Reserved routes such as `admin`, `api`, `auth`, `dashboard`, `login`, `settings`, `support`, `profile`, `organizations`, `productions`, `public`, `system`, `assets`, `backend`, `shared`, `home`, `www`, `mail`, and `app` cannot be generated organisation URLs.
11. Explicit sexual terms, hate speech, slurs, and extreme profanity cannot be generated organisation URLs.
...
- `text` — reusable square layout-holder/ruler template; a temporary grey holder contains the same four visible containers as the Square Brand Tile container breakdown, inset by `7cqw`
