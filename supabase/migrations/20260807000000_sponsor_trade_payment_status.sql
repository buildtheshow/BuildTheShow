-- The internal "Book on the Public Form" popup (embedded admin version of the public
-- sponsor booking flow) adds an "In Trade" checkbox that only shows for producers, never
-- for real sponsors. This threads that choice through to payment_status on the created
-- programme_ads / sponsor_packages row so it shows correctly in the CRM immediately,
-- instead of always landing as "unpaid" until a producer manually flips it.
...
begin
  ...
  if p_payment_status is not null and p_payment_status not in ('unpaid', 'paid', 'invoice_sent', 'overdue', 'trade') then
...
revoke all on function public.finalize_sponsor_submission(uuid,uuid,text,text,text,text,jsonb,jsonb,text) from public;
grant execute on function public.finalize_sponsor_submission(uuid,uuid,text,text,text,text,jsonb,jsonb,text) to anon, authenticated;
