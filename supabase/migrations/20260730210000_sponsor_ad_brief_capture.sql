-- The sponsor public page already collects a full ad design brief (advertising type,
-- "what would you like people to know", what to include, colour/style requests) and the
-- CRM booking card template (crmBookingDetails in marketing-sponsors.js) already renders
-- it from booking.artwork_data -- but no column existed to store it, so the brief was
-- silently discarded after submission and only ever appeared in the outgoing emails.

alter table public.programme_ads add column if not exists artwork_data jsonb not null default '{}'::jsonb;

drop function if exists public.finalize_sponsor_submission(uuid,uuid,text,text,text,text,jsonb);
